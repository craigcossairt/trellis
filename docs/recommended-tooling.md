...
| Design & visuals | Pick one rung, not a shelf. **Never designed anything:** your coding agent + a design skill for in-app UI, plus **Claude Design** or **Google Stitch** for things that are not your app. **Working with a designer, or design is the product:** **Figma**, and **Paper** (paper.design) if you want design and code to be one artifact instead of two that drift. | Most solo founders need less design tooling than they think, and the failure mode is collecting accounts instead of shipping screens. The Impeccable skill above covers in-app UI. Claude Design (Anthropic, research preview) and Google Stitch earn their place for decks, one-pagers and landing mockups - treat their output as a first draft you then tokenise, because both hand you a wall of literal values. Figma when you collaborate with an actual designer. See `DESIGN.md` for the rules that outlast whichever tool you pick. |
